...
def run_benchmark(player_code: str) -> tuple[float, float, float]:
    ...
    cmd.extend([
        ...
        DECK_A, DECK_B,
        "--num", str(NUM_GAMES),
        "--players", f"{player_code},{player_code}",
    ])
    ...
    if time_match:
        total_time_sec = int(time_match.group(1)) * 60 + int(time_match.group(2)) + int(time_match.group(3)) / 1000.0
    else:
        # Try "Xs Xms" format
        time_match = re.search(r"in (\d+)s (\d+)ms", output)
        if time_match:
            total_time_sec = int(time_match.group(1)) + int(time_match.group(2)) / 1000.0
        else:
            # Try "Xms" only format
...
def main():
    ...
    for code, name in PLAYERS:
        ...
        try:
            ...
            print(f"{steps_per_sec:.0f} steps/s (avg {avg_plys:.1f} plys/game, {total_time:.2f}s total)")
    ...
    print("=" * 72)
    print(f"{'Player':<30} {'Code':<6} {'Steps/s':>12} {'Plys/game':>10} {'Time (s)':>10}")
    print("-" * 72)
